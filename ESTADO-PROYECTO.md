...
- Vehículos (F6, para todos desde el 4-sep): carrusel de artes con arrastre, ficha (km, aceite, próximo servicio por fecha/km, tanque, combustible habitual), telemetría (km/gal DE LLENO A LLENO — pregunta "¿llenaste el tanque?" en calificación y registro manual, inferencia ≥85 % del tanque, parciales sumadas a la siguiente ventana; `src/lib/fuelEconomy.js` + `list_my_vehicle_stats`, 21-sep —, km/día, costo por km, gráficas), consumos manuales, historial de cargas por vehículo con reasignación y corrección del estado del tanque, confirmación de servicio desde el push, silencio de recordatorios por vehículo, zona de riesgo para eliminar.
