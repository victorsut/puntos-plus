...
- **Seguridad:** lógica sensible vive en RPCs (`register_purchase`, `redeem_reward`, `authenticate_operator`). **REGLA (SEC.C.7, 21-sep-2026):** toda RPC SECURITY DEFINER que ESCRIBA datos debe validar sesión con `validate_session_token(p_session_token, rol, …)` — recibir `p_admin_id` "para auditoría" NO autentica (cualquiera lo inventa con la llave pública); las funciones internas/helpers llevan REVOKE a anon/authenticated; las vistas van con `security_invoker` y sin grant a anon. Al terminar una migración, correr el diagnóstico: funciones ejecutables por anon sin `validate_session_token` que escriban (`pg_proc` + `has_function_privilege`).
...
- **Tutorial interactivo del cliente (21-sep-2026):** coach marks en `src/components/tour/` (ClientTour motor · ClientTourGate disparo · tourSteps contenido) montados en AppModals; los pasos apuntan a anclas `data-tour="…"` (nav-home/cat/qr/raf/veh, header-help/bell/menu, pts-card, tile-promos/survey…). Al mover o renombrar un botón principal del inicio, conservar su `data-tour`. Aparece al crear la cuenta y en cada inicio de sesión (marca `pp_tour_pending`, `src/lib/tour.js`); reapertura desde Asistencia y ayuda.
