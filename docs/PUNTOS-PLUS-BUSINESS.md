...
- **Puntos (B37):** cada carga de vale se liga al depósito que la cubre (orden de consumo del saldo). Si ese depósito ya está facturado, al consumirse la carga se escribe su `earn` = `round(galones × ptsPerGal)`; si aún no, la carga espera y sus puntos se acreditan en el momento en que el admin registra la factura del depósito (un `earn` por carga, `ref_type = 'fuel_load'` con la factura en el detalle). Así la empresa nunca gana puntos por combustible que no se le ha facturado.
