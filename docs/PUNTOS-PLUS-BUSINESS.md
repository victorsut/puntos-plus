...
## Versión 0.4b · 22 de septiembre de 2026 · estado: DISEÑO ACORDADO Y RECONCILIADO con la recalibración y la API v1.4, EN PAUSA hasta definir el catálogo de premios y los niveles de empresa
...
| B9 | Los **puntos** de las cargas de flota van al **fondo de la empresa**; el chofer no recibe puntos. Se calculan por **galones** con una conversión propia de Business (B19), no por quetzales, y se acreditan **AL FACTURAR, nunca al consumir** (B37). Los canjes de combustible (de la app normal o corporativos) no acreditan puntos a nadie. |
| B10 | Al entregar cualquier canje de combustible (flota o normal, en Puntos Plus y en Business) se abre en el celular del socio el **modal de calificación** con selector de vehículo, kilómetros recorridos y la pregunta "¿Llenaste el tanque?" (F6 E3f), para atribuir la carga a un vehículo; en Business el socio elige entre **sus vehículos propios y los registrados por la empresa** (B38). Vocabulario del cliente: "kilómetros recorridos", nunca "odómetro" (B32). |
...
| B12 | **Facturación** configurable por empresa desde el admin y coordinada con ella: "al depósito" (una factura por depósito, ningún documento por consumo), "por consumo diario" (corte consolidado por tipo de combustible, al final del día o a la hora acordada con la empresa, B26) o, **solo en casos excepcionales, "por consumo"** (una factura por cada carga, B37). Lo ideal es facturar directamente a la empresa. Todas las modalidades llevan control de lo facturado y lo pendiente (B22), y los **puntos de la empresa se acreditan al facturar** (B37). |
...
| # | Ajuste |
|---|---|
| B28 | **Puntos de la empresa con la misma fórmula del programa:** `round(galones × ptsPerGal)` con un `ptsPerGal` propio de Business (`program_config.business_points`), como los tiers desde la recalibración (ORO 3.5 · PLATINO 4.0 · BLACK 4.5). Se descarta `gal_per_pt` con arrastre de fracciones. Valor del punto Q0.10 también para la empresa: precio en puntos de un premio corporativo = valor en Q × 10. El MOMENTO de acreditar es la facturación, no la entrega (B37). |
| B37 | **Los consumos de flota son CANJES de combustible y son independientes de la facturación.** La facturación se coordina con la empresa (por depósito · por consumo diario · excepcionalmente por consumo, una factura por carga; idealmente directa con la empresa) y **los puntos de la empresa se acreditan AL FACTURAR**: el `earn` de `company_points_ledger` se escribe cuando el admin registra la factura emitida (`company_invoices.status = 'issued'`), con los galones de las cargas que esa factura cubre. Consumir con canje NO asigna puntos ni a la empresa ni al piloto, nunca. Modalidad al depósito: la factura se emite antes del consumo, así que los puntos se acreditan con los galones que ese depósito termina cubriendo — se liquidan a medida que sus cargas se consumen (cada carga consumida contra un depósito ya facturado escribe su `earn`; las de un depósito aún sin factura esperan a que se facture). |
| B38 | **Modal de vehículo en TODO canje de combustible, en Puntos Plus y en Business.** Al entregarse un canje de combustible (vale, premio corporativo o premio del catálogo normal) se abre el modal de calificación para asignar el consumo a un vehículo, con los kilómetros recorridos y "¿Llenaste el tanque?". En Business el selector muestra los vehículos propios del socio y los registrados por la empresa a la que está afiliado (preseleccionado el del vale si lo trae). Esa carga alimenta la telemetría del vehículo elegido y **no asigna puntos a nadie**. |
| B39 | **Orden de consumo de los depósitos: el más antiguo primero** (FIFO). Cada carga de vale se cubre con el depósito más antiguo con saldo por consumir; si no alcanza, el resto pasa al siguiente. El estado de cuenta muestra por depósito: monto, facturado o pendiente, consumido y por consumir. |
...
|---|---|---|
| `companies` | La empresa cliente | `id`, `name`, `nit` (UNIQUE, B24), `contact_name`, `contact_phone`, `active`, `station_ids uuid[]` (NULL = todas), `billing_mode` ('deposit' / 'daily' / 'per_load' — al depósito, por consumo diario, por consumo excepcional, B12/B37), `billing_cutoff_time time` (NULL = fin del día, B26), `consumption_invoice_scope` ('vouchers' default / 'all', B20), `reward_invoice_mode` ('cf' default / 'company_nit', B20), `audit_mode` ('basic' / 'strict'), `driver_confirm` (bool, default false), `credit_limit numeric` (default 0), `tier text NULL` (reservado, B23), `created_at` |
...
| `company_points_ledger` | **Libro mayor de PUNTOS** (B19, B25, B37) | `id`, `company_id`, `kind` ('earn' + / 'redeem' − / 'adjustment' ±), `points`, `gallons` (los que generaron el earn), `balance_after`, `ref_type` (el `earn` SIEMPRE referencia una factura `company_invoices` o una carga cubierta por un depósito facturado — nunca una entrega), `ref_id`, `created_by_kind`, `created_by`, `created_at` |
...
3. **Partir** un vale en canjes (B5) y **entregar** una carga NO escriben en `company_ledger` NI en `company_points_ledger`. La entrega solo marca la carga `consumed` y calcula galones; los puntos se acreditan al facturar (B37).
...
3. **Entrega** (`action: 'deliver'`, o el botón Entregar del operador) — una sola transacción, SIN tocar el libro de dinero:
   ...
   - **NO acredita puntos** a la empresa ni al chofer (B37): los galones quedan en `fuel_loads` y los puntos de la empresa nacen al facturar (§6). NO pasa por `register_purchase_core`: sin promociones, sin bono de festivo, sin tasa por tier ni upgrade de tarjeta;
   - inserta la fila en `purchases` (`source = 'voucher'`, `points_earned = 0`, galones calculados, estación, operador, `company_id`, `fuel_load_id`) → el Realtime existente abre el **modal de calificación** en el celular del chofer con el selector de vehículo (el socio elige entre sus vehículos propios y los registrados por la empresa; preseleccionado el del vale si lo trae, B38), los kilómetros recorridos y si el tanque quedó lleno; la telemetría del vehículo se alimenta como con cualquier carga;
...
- **Puntos (B37):** cada carga de vale se liga al depósito que la cubre: **el más antiguo primero** (FIFO por fecha de depósito, decisión del dueño 22-sep, B39); una carga puede repartirse entre dos depósitos si el más antiguo no alcanza a cubrirla, y cada parte acredita sus puntos según el estado de factura de su depósito. Si ese depósito ya está facturado, al consumirse la carga se escribe su `earn` = `round(galones × ptsPerGal)`; si aún no, la carga espera y sus puntos se acreditan en el momento en que el admin registra la factura del depósito (un `earn` por carga, `ref_type = 'fuel_load'` con la factura en el detalle). Así la empresa nunca gana puntos por combustible que no se le ha facturado.
...
- Gerencia emite la factura y el admin anota `invoice_no` → `status = 'issued'`, `issued_at`. **En ese momento se acreditan los puntos (B37):** un `earn` en `company_points_ledger` con `ref_type = 'company_invoice'`, galones = suma de las cargas de VALE del corte y `points = round(galones × ptsPerGal)`; las cargas de premio dentro del corte (`scope = 'all'`) no generan puntos.
- **Por consumo excepcional** (`billing_mode = 'per_load'`, B12): cada carga consumida crea de inmediato su `company_invoices` en `draft` (una factura por carga); al anotar el número de factura se acreditan sus puntos igual que en un corte. Se reserva para empresas que lo exijan; lo normal es el corte diario o el depósito.
...
- **v0.4b (22-sep-2026, tarde):** precisiones del dueño B37-B39 (depósitos se consumen del más antiguo al más nuevo): los consumos de flota son canjes de combustible independientes de la facturación (por depósito · por consumo diario · excepcionalmente por consumo, una factura por carga; idealmente directa con la empresa) y los **puntos de la empresa se acreditan AL FACTURAR, nunca al consumir**; el modal de vehículo (propio o de la empresa, kilómetros recorridos, tanque lleno) se abre en TODO canje de combustible de Puntos Plus y de Business y no asigna puntos a nadie. `billing_mode` pasa a deposit/daily/per_load; el `earn` referencia facturas o cargas cubiertas por depósitos facturados.
